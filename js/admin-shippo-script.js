...
jQuery(document).ready(function($) {
    ...
    const defaultProductPackagingRules = {
        'default': { length: 10, width: 8, height: 6, weight: 32 }, // Default weight in ounces (2 lbs)
    ...
    // Input for weight in ounces

    const packageWeightLbDisplay = $('#wc_shippo_package_weight_lb'); // New element for pounds display
    ...
    const carrierSelectionBox = $('#carrier-selection-box');; // Timeout for saving weight changes


    let selectedRate = null; // To store the currently selected shipping rate

    /* Converts weight from ounces to pounds.
     * @param {number} oz The weight value in ounces.
    ...
    /**
     * Converts weight from WooCommerce's default unit to ounces.
     * @param {number} weight The weight value.
     * @param {string} unit The WooCommerce weight unit (e.g., 'kg', 'g', 'lbs', 'oz').
    ...
    function convertWeightToOunces(weight, unit) {
       ...
       switch (unit.toLowerCase()) {
            case 'kg':
                return weight * 35.274; // 1 kg = 35.274 oz
            case 'g':
                return weight * 0.035274; // 1 g = 0.035274 oz
            case 'lbs':
                return weight * 16; // 1 lbs = 16 oz
            case 'oz':
            default:
                return weight;
        }
     ...
     * Updates the side-by-side pound display for the weight input.

     */

    function updateWeightLbDisplay() {
        const ozValue = parseFloat(packageWeightInput.val());
        if (!isNaN(ozValue) && ozValue >= 0) {
            const lbValue = convertOzToLbs(ozValue);
            packageWeightLbDisplay.text(`(${lbValue.toFixed(2)} lb)`);
        } else {
            packageWeightLbDisplay.text('');
        }
    }
    ...
    function toggleDimensionFields(type) {
        if (type === 'flat_rate') {
            ...
            carrierSelectionBox.hide(); // Hide carrier selection for flat rate
            
        } else { // 'custom'
            customDimensionsFields.show();
            carrierSelectionBox.show();
    ...
    function updatePackageDetails() {
        ...
        // Update the pounds display immediately

        updateWeightLbDisplay();
    ...
    // Event listener for weight input to update pounds display and save
    packageWeightInput.on('input', function() {
        updateWeightLbDisplay(); // Update pounds display immediately
        clearTimeout(saveWeightTimeout); // Clear previous timeout
        saveWeightTimeout = setTimeout(savePackagingRule, 1000); // Save after 1 second of no further input
